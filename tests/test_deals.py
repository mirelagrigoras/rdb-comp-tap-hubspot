from rdb_comp_tap_hubspot import sync_deals
import unittest
...
class TestDealsToSync(unittest.TestCase):
    ...
    @patch('rdb_comp_tap_hubspot.Context.get_catalog_from_id', return_value={"metadata":""})
    @patch('singer.metadata.to_map', return_value={})
    @patch('singer.utils.strptime_with_tz')
    @patch('singer.utils.strftime')
    @patch('rdb_comp_tap_hubspot.load_schema')
    @patch('rdb_comp_tap_hubspot.gen_request', return_value=list())
